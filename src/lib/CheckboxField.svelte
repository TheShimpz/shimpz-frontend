...
<style>
  label { position: relative; display: grid; grid-template-columns: 1.125rem minmax(0, 1fr); align-items: start; gap: 0.6rem; cursor: pointer; }
  ...
  .copy { display: grid; gap: 0.15rem; } strong { color: var(--shimpz-color-text); font: 600 0.8rem/1.35 var(--shimpz-font-mono); } small { color: var(--shimpz-color-text-muted); font-size: 0.78rem; line-height: 1.4; }
